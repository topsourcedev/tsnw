(function() {
    var app = angular.module("AvanzamentoProgetto", [], function($interpolateProvider) {
...
//        $httpProvider.defaults.headers.post['Content-Type'] = 'application/x-www-form-urlencoded;charset=utf-8';
    ...
    app.filter("filterOnStatus", function() {
        return function(input, status) {
            ...
            if (status + "" === "0") {
                ...
                for (var i in input) {
                    if (input[i].status + "" === "1" && input[i].flowstatus + "" === "0") {
                        ret.push(input[i]);
                ...
                for (var i in input) {
                    if (input[i].status + "" === "1" && input[i].flowstatus + "" === "2") {
                        ret.push(input[i]);
    ...
    app.controller("CommentiController", function($rootScope, $http, $timeout) {
        ...
        this.push = function() {
            ...
            url = url.replace("#", "").replace("avanzamento", "commenti");
            url = url + ".ws";
    ...
    app.controller("PulsantieraController", function($rootScope, $http) {
        ...
        this.salva = function() {
            ...
            $rootScope.flowerizzatore = 0;
            this.salvaEsecuzione();
//            while ($rootScope.flowerizzatore < 1) {
//            }
            this.salvaAttivita();
//            while ($rootScope.flowerizzatore < 2) {
//            }
            this.salvaFatturazione();
//            while ($rootScope.flowerizzatore < 3) {
//            }
            this.salvaIncasso();
        };
...
//                        }
                    ...
                    $timeout(function() {
                        ...
                        urlContatti = urlContatti.split("avanzamento/");
...
//                    pippo.data = $scope.rawdata;
        ...
        $timeout(function() {
            var url = document.URL.replace("#", "").replace("avanzamento", "avanzamentoMoreInfo") + '.ws';
            var responsePromise = $http.get(url)
                    .success(function(data, status, headers, config) {
                        ...
                        for (var i in $rootScope.infoEsecuzione) {
                            ...
                            if ($rootScope.infoEsecuzione[i].isactual === 1 && $rootScope.infoEsecuzione[i].status + "" === "1" && $rootScope.infoEsecuzione[i].flowstatus + "" === "0") {
                        ...
                        for (var i in $rootScope.infoFatturazione) {
                            ...
                            if ($rootScope.infoFatturazione[i].isactual === 1 && $rootScope.infoFatturazione[i].status + "" === "1" && $rootScope.infoFatturazione[i].flowstatus + "" === "0") {
                        ...
                        for (var i in $rootScope.infoIncasso) {
                            ...
                            if ($rootScope.infoIncasso[i].isactual === 1 && $rootScope.infoIncasso[i].status + "" === "1" && $rootScope.infoIncasso[i].flowstatus + "" === "0") {
    ...
    app.controller("EsecuzioneController", function($rootScope, $http) {
        ...
        this.show = 0; //0= mostra budget, 1 = mostra forecast, 2 = mostra forecast respinto
        this.changeShow = function(show) {
            this.show = show;
        }
        this.showClass = function(tab) {
        ...
        this.add = function() {
            ...
            $rootScope.nuovo.esecuzione.push({"date": "", "label": "", "status": "1", "isactual": 0, "actualdate": "", "actuallabel": "", flowstatus: "0"});
        }
    });
})();
